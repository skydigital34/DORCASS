 ...
 */
export const saveStoredProducts = (products, shouldNotify = true) => {
  try {
...
export const initFirestoreSync = () => {
  ...
  if (isFirebaseConfigured()) {
    firestoreUnsubscribe = subscribeToFirestoreProducts((firestoreProducts) => {
      if (Array.isArray(firestoreProducts) && firestoreProducts.length > 0) {
        saveStoredProducts(firestoreProducts, true);
      }
    });
  }
};

// Start Firestore Sync if Firebase is configured
initFirestoreSync();
...
/**
 * Fetch all products asynchronously (from Firestore or LocalStorage)
 */
export const fetchProducts = async () => {
  if (isFirebaseConfigured()) {
    try {
      const remote = await fetchAllFirestoreProducts();
      if (remote && Array.isArray(remote) && remote.length > 0) {
        saveStoredProducts(remote, false);
        return remote;
      }
    } catch (err) {
...
/**
 * Create a new product (Saves locally and syncs to Firebase Firestore)
 */
export const createProduct = async (productData) => {
  ...
  try {
    ...
    console.warn('Syncing to Firestore failed, product preserved locally:', err);
  }
...
/**
 * Update an existing product (Saves locally and syncs to Firebase Firestore)
 */
export const updateProduct = async (id, updateData) => {
  ...
  try {
    ...
    console.warn('Syncing updated product to Firestore failed:', err);
  }
...
/**
 * Delete a product by ID (Deletes locally and from Firebase Firestore)
 */
export const deleteProduct = async (id) => {
  ...
  try {
    ...
    console.warn('Deleting product from Firestore failed:', err);
  }
